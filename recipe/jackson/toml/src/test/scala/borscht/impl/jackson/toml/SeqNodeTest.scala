...
class SeqNodeTest extends AnyFlatSpec with Matchers:
  ...
  it should "provide a singleton list of numbers" in {
    cfg"key = ${Int.MaxValue}"[List[BigDecimal]]("key") should contain theSameElementsInOrderAs List(Int.MaxValue)
  }
  ...
  it should "provide a list of booleans" in {
    cfg"key = [true, true, false]"[List[Boolean]]("key") should contain theSameElementsInOrderAs

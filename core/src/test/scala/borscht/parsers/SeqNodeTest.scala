...
class SeqNodeTest extends AnyFlatSpec with Matchers:
  ...
  it should "provide a list of numbers" in {
    cfg("key" -> seq(42, 2.66))[List[Number]]("key") must contain theSameElementsInOrderAs
      List(42, 2.66)
  }
  ...
  it should "provide a list of booleans" in {
    cfg("key" -> seq(true, true, false))[List[Boolean]]("key") must contain theSameElementsInOrderAs
      List(true, true, false)
  }

  it should "provide a list of booleans from a string sequence" in {
    cfg("key" -> seq("false", "true", "false"))[List[Boolean]]("key") must contain theSameElementsInOrderAs

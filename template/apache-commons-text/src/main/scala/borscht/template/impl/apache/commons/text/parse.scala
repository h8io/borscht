...
private[text] def parse(chars: String, valueFormat: ValueFormat): (String, ValueFormat) =
  ...
  def skipWS(): Unit = if hasNext then i = chars.indexWhere(!_.isWhitespace, i)

  def expectEqual(): Unit =
    skipWS()
    if hasNext then
      if next() != '=' then error("'=' expected") else ()
  ...
  def getString: Option[String] =
    ...
    if hasNext then
      Some((next() match
        ...
        case c   => getBareString(StringBuilder(), c)
      ).result())
    else None
  ...
  def getBareString(sb: StringBuilder, c: Char): StringBuilder = c match
    ...
    case '=' =>
      i -= 1
      sb
    case c if isValidBareChar(c) => if hasNext then getBareString(sb += c, next()) else sb += c
    case c                       => error(s"Unexpected char '$c' in the bare string")

  def isValidBareChar(c: Char): Boolean = c != '\\' && c != '"'

  @tailrec
  def getQuotedString(sb: StringBuilder): StringBuilder = if hasNext then
    next() match
      case '"'  => sb
  ...
  def getCharByCode: Char =
    ...
    def loop(code: Int, i: Int = 4): Int =
      if i == 0 then code else loop(code * 16 + Character.digit(next(), 16), i - 1)

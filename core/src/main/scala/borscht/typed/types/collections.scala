...
object RefTypeList extends RefTypeWithOptionalParameter:
  override protected def create(parser: NodeParser[Ref[?]]): NodeParser[Ref[List[?]]] = node =>
    node match
...
object RefTypeMap extends AbstractRefType[(NodeParser[Ref[?]], NodeParser[Ref[?]])]:
  override protected def prepare(
      parameters: List[NodeParser[Ref[?]]]
  ): Either[String, (NodeParser[Ref[?]], NodeParser[Ref[?]])] =
    parameters match
  ...
  override protected def create(parsers: (NodeParser[Ref[?]], NodeParser[Ref[?]])): NodeParser[Ref[Map[?, ?]]] =
    ...
    _ match
      case cfg: CfgNode =>
        Ref((cfg.iterator map { (key, value) =>
...
object RefTypeOption extends RefTypeWithOptionalParameter:
  override protected def create(parameter: NodeParser[Ref[?]]): NodeParser[Ref[Option[?]]] = node =>
    node match

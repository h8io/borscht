...
class RefObj[T <: AnyRef](_value: => T)(using val classTag: ClassTag[T]) extends Ref[T]:
  ...
  def cast[R](using tag: ClassTag[R]): Ref[R] =
    ...
    if r.isAssignableFrom(t) then asInstanceOf[Ref[R]]
    else
      RefObj.UnboxMap.get(t) collect {
...
object RefObj:
  private def lift[B <: AnyRef, P <: Primitive](
      f: B => P
...
final class RefComponent[T <: AnyRef: ClassTag](value: => T) extends RefObj(value):
  def assign[R <: AnyRef](using tag: ClassTag[R]): RefComponent[R] =
    val r = tag.runtimeClass
    val t = classTag.runtimeClass
    if r.isAssignableFrom(t) then asInstanceOf[RefComponent[R]]
...
final class RefBoolean(_value: => Boolean) extends RefPrimitive(_value):
  override protected def boxedClass: Class[lang.Boolean] = classOf[java.lang.Boolean]

final class RefByte(_value: => Byte) extends RefPrimitive(_value):
  override protected def boxedClass: Class[lang.Byte] = classOf[java.lang.Byte]

final class RefChar(_value: => Char) extends RefPrimitive(_value):
  override protected def boxedClass: Class[Character] = classOf[java.lang.Character]

final class RefDouble(_value: => Double) extends RefPrimitive(_value):
  override protected def boxedClass: Class[lang.Double] = classOf[java.lang.Double]

final class RefFloat(_value: => Float) extends RefPrimitive(_value):
  override protected def boxedClass: Class[lang.Float] = classOf[java.lang.Float]

final class RefInt(_value: => Int) extends RefPrimitive(_value):
  override protected def boxedClass: Class[Integer] = classOf[java.lang.Integer]

final class RefLong(_value: => Long) extends RefPrimitive(_value):
  override protected def boxedClass: Class[lang.Long] = classOf[java.lang.Long]

final class RefShort(_value: => Short) extends RefPrimitive(_value):
  override protected def boxedClass: Class[lang.Short] = classOf[java.lang.Short]

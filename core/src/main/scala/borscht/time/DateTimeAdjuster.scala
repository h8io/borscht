...
import java.time.temporal.{ChronoUnit, Temporal, TemporalAdjuster, TemporalAmount, TemporalUnit}
import java.util
...
object DateTimeAdjuster extends (String => DateTimeAdjuster):
  ...
  def apply(value: String): DateTimeAdjuster = if (value.isEmpty) ZeroDateTimeAdjuster else value match
  ...
  private def apply(sign: String,
                    ...
                    hours: String | Null, minutes: String | Null, seconds: String | Null): DateTimeAdjuster = apply(
    sign,
